...
let state = {
    ...
    showPeso: true, showIntensidad: true, weeks: 4, chartInstance: null 
};
...
// ==========================================================================
//   LÓGICA DEL TELÓN INVERSO Y GRÁFICO (CHART.JS)
// ==========================================================================

// --- FUNCIONES AUXILIARES PARA EL FAB ---
function hideFabForChart() {
    const fab = document.getElementById('fab-container');
    if(fab) fab.classList.add('fade-out');
}
function showFabForChart() {
    const fab = document.getElementById('fab-container');
    if(fab) fab.classList.remove('fade-out');
}

// --- CLIC EN LA PESTAÑA ---
function toggleBottomSheet() {
    const sheet = document.getElementById('progression-sheet');
    sheet.classList.toggle('open');
    sheet.style.transform = ''; 
    
    if(sheet.classList.contains('open')) hideFabForChart();
    else showFabForChart();
}

function initChartSheet() {
    const routineSelect = document.getElementById('routine-select');
    const exerciseSelect = document.getElementById('exercise-select');
    const togglePeso = document.getElementById('toggle-peso');
    const toggleIntensidad = document.getElementById('toggle-intensidad');
    const slider = document.getElementById('time-slider');
    const timeLabel = document.getElementById('time-label');

    // 1. Limpiar selectores
    routineSelect.innerHTML = '<option value="" disabled selected>Rutina...</option>';
    
    // 2. Llenar selector de rutinas únicas
    const uniqueRoutines = Array.from(new Set(state.libRoutines.map(r => r.ID_entreno)))
                                .map(id => state.libRoutines.find(r => r.ID_entreno === id));
    
    uniqueRoutines.forEach(r => {
        const opt = document.createElement('option');
        opt.value = r.ID_entreno;
        opt.textContent = r.Nombre_rutina;
        routineSelect.appendChild(opt);
    });

    routineSelect.addEventListener('change', (e) => {
        const rId = e.target.value;
        const rData = state.libRoutines.find(r => r.ID_entreno === rId);
        
        exerciseSelect.innerHTML = '<option value="" disabled selected>Ejercicio...</option>';
        exerciseSelect.disabled = false;

        for(let i=1; i<=15; i++) {
            const exId = rData[`Ejercicio_${String(i).padStart(2,'0')}`];
            if(exId) {
                const exInfo = state.libExercises.find(ex => ex.ID_ejercicio === exId);
                if(exInfo) {
                    const opt = document.createElement('option');
                    opt.value = JSON.stringify({ id: exId, pos: String(i).padStart(2,'0') });
                    opt.textContent = `${i}. ${exInfo.Nombre_ejercicio}`;
    ...
    // 3. Lógica de las pastillas (con bloqueo de seguridad)
    togglePeso.addEventListener('click', () => {
        if (state.showPeso && !state.showIntensidad) {
            showToast("Debes mantener al menos una métrica activa");
            return;
        }
        state.showPeso = !state.showPeso;
        togglePeso.classList.toggle('active', state.showPeso);
        updateChart();
    });

    toggleIntensidad.addEventListener('click', () => {
        if (state.showIntensidad && !state.showPeso) {
            showToast("Debes mantener al menos una métrica activa");
            return;
        }
        state.showIntensidad = !state.showIntensidad;
        toggleIntensidad.classList.toggle('active', state.showIntensidad);
        updateChart();
    });

    slider.addEventListener('input', (e) => {
        state.weeks = parseInt(e.target.value);
        timeLabel.textContent = `Últimas ${state.weeks} sem.`;
        updateChart();
    });

    // 4. LÓGICA DE ARRASTRE (SWIPE) DE LA PERSIANA
    const sheet = document.getElementById('progression-sheet');
    ...
    pullTab.addEventListener('touchmove', (e) => {
        const deltaY = e.touches[0].clientY - startY;
        
        // Si la persiana está bajada y tiramos hacia arriba (deltaY negativo)
        if (!sheet.classList.contains('open') && deltaY < 0) {
            // Límite del 65% de la pantalla para coincidir con tu CSS de 65vh
            const move = Math.max(deltaY, -(window.innerHeight * 0.65));
            sheet.style.transform = `translateY(calc(100% - 50px + ${move}px))`;
            
            // Oculta el botón FAB al subir la persiana
            if (deltaY < -20) hideFabForChart();
            
        // Si la persiana está arriba y empujamos hacia abajo (deltaY positivo)
        } else if (sheet.classList.contains('open') && deltaY > 0) {
            sheet.style.transform = `translateY(${deltaY}px)`;
        }
    }, { passive: true });

    pullTab.addEventListener('touchend', (e) => {
        sheet.style.transition = 'transform 0.5s cubic-bezier(0.19, 1, 0.22, 1)';
        const deltaY = e.changedTouches[0].clientY - startY;
        
        // Decidir si abre o cierra en función de la inercia (> 50px)
...
function updateChart() {
    const rId = document.getElementById('routine-select').value;
    const exVal = document.getElementById('exercise-select').value;
    ...
    let dataPoints = state.logs.filter(l => 
        l.ID_entreno === rId && 
        l.ID_log_train_ej.endsWith(posToMatch) && 
        new Date(l.Fecha_log_entreno) >= cutoffDate
    );

    // Ordenar de más antiguo a más reciente
    dataPoints.sort((a,b) => new Date(a.Fecha_log_entreno) - new Date(b.Fecha_log_entreno));

    if (dataPoints.length === 0) {
        emptyState.textContent = "No hay datos para este ejercicio en el periodo seleccionado.";
        emptyState.classList.remove('hidden');
        if(state.chartInstance) { state.chartInstance.destroy(); state.chartInstance = null; }
        return;
    }
    ...
    dataPoints.forEach(log => {
        ...
        for(let i=1; i<=5; i++) {
            ...
            let r = parseInt(log[`Log_repes_serie_0${i}`]);
    ...
    // Calcular la evolución relativa (100% es el primer día que aparece en la gráfica)
    let baseIntensidad = rawIntensidad[0] || 1;
    let intensidadData = rawIntensidad.map(val => ((val / baseIntensidad) * 100).toFixed(1));

    renderChart(labels, pesoData, intensidadData);
}

function renderChart(labels, pesoData, intensidadData) {
    const ctx = document.getElementById('progressionChart').getContext('2d');
    if (state.chartInstance) state.chartInstance.destroy();

    const gradientPeso = ctx.createLinearGradient(0, 0, 0, 300);
    gradientPeso.addColorStop(0, 'rgba(52, 199, 89, 0.4)');
    gradientPeso.addColorStop(1, 'rgba(52, 199, 89, 0.0)');

    const gradientIntensidad = ctx.createLinearGradient(0, 0, 0, 300);
    gradientIntensidad.addColorStop(0, 'rgba(255, 149, 0, 0.4)');
    gradientIntensidad.addColorStop(1, 'rgba(255, 149, 0, 0.0)');

    const datasets = [];

    if (state.showPeso) {
        datasets.push({
            label: 'Carga Máx (Kg)', data: pesoData, borderColor: '#34C759',
            backgroundColor: gradientPeso, borderWidth: 3, pointBackgroundColor: '#fff',
            pointBorderColor: '#34C759', pointBorderWidth: 2, pointRadius: 4, fill: true,
            yAxisID: 'yPeso', tension: 0.4
        });
    }

    if (state.showIntensidad) {
        datasets.push({
            label: 'Intensidad Relativa (%)', data: intensidadData, borderColor: '#FF9500',
            backgroundColor: gradientIntensidad, borderWidth: 3, pointBackgroundColor: '#fff',
            pointBorderColor: '#FF9500', pointBorderWidth: 2, pointRadius: 4, fill: true,
            yAxisID: 'yIntensidad', tension: 0.4
        });
    }
    ...
    state.chartInstance = new Chart(ctx, {
        ...
        options: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            if (label) label += ': ';
                            if (context.dataset.yAxisID === 'yIntensidad') label += context.parsed.y + '%';
                            else label += context.parsed.y + ' kg';
                            return label;
                        }
                    }
                }
            ...
            scales: {
                x: { grid: { display: false }, ticks: { font: { family: 'Outfit', weight: 'bold' }, color: '#8e8e93' } },
                yPeso: {
                    type: 'linear', display: state.showPeso, position: 'left', grid: { color: 'rgba(0,0,0,0.05)' },
                    title: { display: true, text: 'Kg', color: '#34C759', font: {family: 'Outfit', weight:'bold'} }
                },
                yIntensidad: {
                    type: 'linear', display: state.showIntensidad, position: 'right', grid: { drawOnChartArea: false }, 
                    title: { display: true, text: '% Evolución', color: '#FF9500', font: {family: 'Outfit', weight:'bold'} }
                }
            }
        }
    });
